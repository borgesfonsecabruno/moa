 ...
 */
public class MinasClustream extends AbstractClusterer{
	...
	private List<Integer> groupBuffer;

	public void setBufferSize(int bufferSize) {
		this.bufferSize = bufferSize * maxNumKernelsOption.getValue();
	}
...
//			Clustering kmeans_clustering = kMeans(k, buffer);
		...
		*/
		...
		groupBuffer.add(closestB);
	}

	public List<Integer> getGroupBuffer() {
		return groupBuffer;
	}
	...
	public Clustering kMeans( int k, Cluster[] centers, List<? extends Cluster> data ) {
		...
		while ( repetitions-- >= 0 ) {
			...
			for ( Cluster point : data ) {
				...
				if(repetitions == -1) {
					groupBuffer.add(closestCluster);
				}
